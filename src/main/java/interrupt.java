...
public class interrupt extends javax.swing.JFrame {
      PHH2 processFrame= new PHH2();
     public interrupt(){
         initComponents();
     }
     public interrupt(PHH2 processFrame) {
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
                String processID = jTextField1.getText();
        if (jTextField1.getText().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please enter a process ID to interrupt.", "Error", JOptionPane.ERROR_MESSAGE);
        } else if (!jRadioButton1.isSelected() && !jRadioButton2.isSelected() && !jRadioButton3.isSelected()) {
            JOptionPane.showMessageDialog(this, "Please select a device (Mouse, Keyboard, or Printer).", "Error", JOptionPane.ERROR_MESSAGE);
        } else {
            if (processFrame != null && processFrame.containsProcessID(processID)) {
                processFrame.changeProcessStatus(processID, "blocked", "Low");
                JOptionPane.showMessageDialog(this, "Interrupt created successfully.", "Success", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this, "Process with ID " + processID + " does not exist.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
        jTextField1.setText("");
   ...
   String processID = jTextField2.getText();

    if (processID.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Please enter your Process ID");
    } else {
        if (processFrame != null && processFrame.containsProcessID(processID)) {
            if (processFrame.isInterruptedIn(processID)) {
                processFrame.changeProcessStatus(processID, "Ready", "Medium");
                JOptionPane.showMessageDialog(this, "Interrupt removed and process is now in ready state");
            } else {
                JOptionPane.showMessageDialog(this, "No interrupt has occurred for process ID: " + processID);
            }
        } else {
            JOptionPane.showMessageDialog(this, "Process with ID " + processID + " does not exist.", "Error", JOptionPane.ERROR_MESSAGE);
        }
